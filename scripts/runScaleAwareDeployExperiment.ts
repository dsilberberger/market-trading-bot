...
type VariantKey = 'baseline' | 'deploy_adjusted';
...
interface WindowSummary {
  ...
  scales: Array<{
    ...
    deployAdjusted: VariantRunSummary;
    benchmarks: Array<{
      portfolio: string;
...
const OUTPUT_ROOT = path.join(ROOT, 'research/broad_validation/runs/deploy_adjustment_scale_aware_experiment');
const SCALES = [2, 3];
const VARIANTS: VariantSpec[] = [
  ...
  {
    key: 'deploy_adjusted',
    label: 'Higher risk_on deploy',
    configPath: path.join(ROOT, 'src/config/default.higher_risk_on_deploy.position_size_scaled_risk_gate.json')
  }
];
...
const buildMemo = (windows: WindowSummary[]) => {
  const lines: string[] = ['# Higher Deploy Scale-Aware Memo', ''];
  ...
  for (const window of windows) {
    ...
    for (const scaleSummary of window.scales) {
      ...
      lines.push(
        `Risk-on occupancy stayed ${(scaleSummary.baseline.riskOnOccupancyPct * 100).toFixed(2)}% -> ${(scaleSummary.deployAdjusted.riskOnOccupancyPct * 100).toFixed(2)}%.`
      );
      lines.push(
        `Average realized equity moved ${(scaleSummary.baseline.averageRealizedEquityAllocation * 100).toFixed(2)}% -> ${(scaleSummary.deployAdjusted.averageRealizedEquityAllocation * 100).toFixed(2)}%, and average core cash moved ${(scaleSummary.baseline.averageCoreCashPct * 100).toFixed(2)}% -> ${(scaleSummary.deployAdjusted.averageCoreCashPct * 100).toFixed(2)}%.`
      );
      lines.push(
        `Return moved ${(scaleSummary.baseline.strategyReturnPct * 100).toFixed(2)}% -> ${(scaleSummary.deployAdjusted.strategyReturnPct * 100).toFixed(2)}% with max drawdown ${(scaleSummary.baseline.maxDrawdownPct * 100).toFixed(2)}% -> ${(scaleSummary.deployAdjusted.maxDrawdownPct * 100).toFixed(2)}%.`
      );
      if (baseline6040) {
        lines.push(
          `Deploy-adjusted versus 60/40 remains ${(scaleSummary.deployAdjusted.strategyReturnPct * 100).toFixed(2)}% vs ${(baseline6040.totalReturnPct * 100).toFixed(2)}%.`
        );
      }
      lines.push('');
    }
  ...
  lines.push(
    'If higher risk_on deploy materially raises realized equity and closes benchmark gaps without disproportionate drawdown, insufficient deployment is a primary remaining driver of conservatism.'
  );
...
const run = async () => {
    ...
    for (const scale of SCALES) {
      ...
      for (const variant of VARIANTS) {
        ...
        const result = await runHistoricalReplay({
          ...
          outputDir,
          runPrefix: `replay-deploy-adjustment-${window.key}-${variant.key}-scale${scale}x`,
      ...
      scaleSummaries.push({
        ...
        deployAdjusted: results.deploy_adjusted,
        benchmarks,
        delta: {
          riskOnOccupancyPct: round(results.deploy_adjusted.riskOnOccupancyPct - results.baseline.riskOnOccupancyPct),
          averageRealizedEquityAllocation: round(
            results.deploy_adjusted.averageRealizedEquityAllocation - results.baseline.averageRealizedEquityAllocation
          ),
          averageCoreCashPct: round(results.deploy_adjusted.averageCoreCashPct - results.baseline.averageCoreCashPct),
          strategyReturnPct: round(results.deploy_adjusted.strategyReturnPct - results.baseline.strategyReturnPct),
          maxDrawdownPct: round(results.deploy_adjusted.maxDrawdownPct - results.baseline.maxDrawdownPct),
          plannerUnexecutableSteps: results.deploy_adjusted.plannerUnexecutableSteps - results.baseline.plannerUnexecutableSteps,
          approvedBuyNotionalPctOfPlanned: round(
            results.deploy_adjusted.approvedBuyNotionalPctOfPlanned - results.baseline.approvedBuyNotionalPctOfPlanned
          )
        }
      });
    }
  ...
  writeJson(summaryPath, {
    ...
    experiment: 'deploy_adjustment_scale_aware',
    scales: SCALES,
  ...
  for (const window of windowSummaries) {
    for (const scaleSummary of window.scales) {
      const rows: Array<[string, VariantRunSummary]> = [
        ...
        ['deploy_adjusted', scaleSummary.deployAdjusted]
      ];
  ...
  console.log(`Deploy adjustment summary: ${summaryPath}`);
};

run().catch((error) => {
  console.error('deploy adjustment scale-aware experiment failed', error);
  process.exitCode = 1;
});
